...
public interface FundingMapper {
    ...
    @Select({"<script>" ,
		"select f.fund_no, f.member_id, f.fund_subject, f.fund_count, f.sheltername, f.start_date, f.end_date , datediff(f.end_date,now()) restdate, if(ROUND(sum(l.fund_cost)*100/(f.fund_count))>100,100,ROUND(sum(l.fund_cost)*100/(f.fund_count))) 'complete' from funding f LEFT OUTER JOIN fundinglist l ON f.fund_no = l.fund_no",
