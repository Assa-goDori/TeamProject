...
<table>
	<tr><td colspan="5">
		<div style="display: inline;">
			<form action="reviewList.shop" method="post" name="searchform">
				...
				<input type="button" value="전체 게시물 보기" class="g_btn" onclick="location.href='reviewList.dog'">
			</form>	
		</div></td>
	</tr>
	...
	<c:if test="${listcount > 0 }">  
		...
		<c:forEach var="board" items="${boardlist }" varStatus="stat">
			<div class="rl_div">
				<table>
					<tr><td>${board.file1 }</td></tr>
					<tr><th><a href="reviewDetail.shop?no=${board.board_no }">${board.subject }</a></th></tr>
					<tr><td><fmt:formatDate value="${board.regdate }" pattern="yyyy-MM-dd HH:mm"/></td></tr>
					<tr><td>${board.member.id }</td></tr>
				</table>	
			</div>
